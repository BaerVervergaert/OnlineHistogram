...
class HoeffdingLebesgueOnlineHistogramAlgorithm(BaseOnlineHistogramAlgorithm):
    def __init__(self,box_bound):
        super(HoeffdingLebesgueOnlineHistogramAlgorithm,self).__init__()
        self.box_bound = box_bound
        self._initialize_states()
        self.p_bound =
    def _initialize_states(self):
    ...
    def constraint(self,state,x):
        if isinstance(state,OneDimensionalInterval):
            if state.parent_state is not None:
                parent_count = state.parent_count.latest_count
            else:
                parent_count = 0
            live_count = self.count-parent_count
            if state.count/live_count > 1/len(self.states):
                out = live_count>=np.log(1/self.p_bound)*8/((state.count/live_count - 1/len(self.states))**2)
            else:
                out = False
            return(out)
        else:
